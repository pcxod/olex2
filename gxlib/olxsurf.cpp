  ...
  radii(atoms.Count(), false)
{
  ...
  for (size_t i = 0; i < crds.Count(); i++) {
    ...
    for (size_t i1 = 0; i1 < mask.width; i1++) {
      for (size_t i2 = 0; i2 < mask.height; i2++) {
        for (size_t i3 = 0; i3 < mask.depth; i3++) {
          if (!mask[i1][i2][i3]) {
            continue;
          }
          const int sm[2] = { -1, 1 };
          for (size_t j1 = 0; j1 < 2; j1++) {
            for (size_t j2 = 0; j2 < 2; j2++) {
              for (size_t j3 = 0; j3 < 2; j3++) {
                int x = v[0] + (int)i1*sm[j1],
                  y = v[1] + (int)i2*sm[j2],
                  z = v[2] + (int)i3*sm[j3];
                if (x >= 0 && (size_t)x < arr.width &&
                  y >= 0 && (size_t)y < arr.height &&
                  z >= 0 && (size_t)z < arr.depth)
                {
                  arr[x][y][z] = 0;
                }
              }
