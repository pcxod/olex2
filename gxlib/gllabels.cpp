...
bool TXGlLabels::Orient(TGlPrimitive& P) {
  ...
  if ((Mode&lmBonds) == 0) {
    ...
    for (size_t i = 0; ai.HasNext(); i++) {
      ...
      if ((Mode & lmQPeak) == 0 && (XA.GetType() == iQPeakZ)) {
        continue;
      }
      if ((Mode & lmIdentity) != 0 && !XA.IsAUAtom()) {
        continue;
      }
      ...
      if ((Mode & lmSpec) != 0 && ca.GetSpecialPositionDeviation() > 1e-3) {
        Tmp << ", "
          << olxstr::FormatFloat(3, ca.GetSpecialPositionDeviation()).TrimFloat();
      }
      ...
      if ((Mode & lmFixed) != 0) {
        ...
        for (size_t j = 0; j < 3; j++) {
          if (ca.GetVarRef(catom_var_name_X + j) != 0 &&
        ...
        if (ca.GetEllipsoid() != 0) {
          ...
          for (size_t j = 0; j < 6; j++) {
            if (ca.GetVarRef(catom_var_name_U11 + j) != 0 &&
        ...
        else if (ca.GetVarRef(catom_var_name_Uiso) != 0 &&
    ...
    for (size_t i = 0; bi.HasNext(); i++) {
      TXBond &b = bi.Next();
      if (!b.IsVisible() || Marks[i] == lmiMasked) {
        continue;
      }
      if ((Mode & lmHydr) == 0 &&
