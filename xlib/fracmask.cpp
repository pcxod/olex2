...
void FractMask::Init(const vec3d& _min, const vec3d& _max,
  const vec3d& norms, double resolution)
{
  if (Mask != NULL) {
    delete Mask;
  ...
  const TVector3<index_t> min_((_min*Norm).Round<index_t>()),
    max_((_max*Norm).Round<index_t>());
...
void FractMask::ToDataItem(TDataItem& di, IOutputStream& os) const {
  ...
  di.AddField("min", PersUtil::VecToStr(TVector3<index_t>(Mask->GetMin1(),
    Mask->GetMin2(), Mask->GetMin3())));
  di.AddField("max",
