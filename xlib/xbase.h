...
protected:
  ...
  size_t OwnerId, IdInNetwork;
public:
  TSObject(Net* Parent)
    : Network(Parent), Type(sotNone), OwnerId(~0)
  {}
  virtual ~TSObject()  {}
  void Assign(const TSObject& S) {
    ...
    IdInNetwork = S.GetIdInNetwork();
  }
  DefPropP(short, Type)
  // this must be updated by the owner container
  DefPropP(size_t, OwnerId)
  DefPropP(size_t, IdInNetwork)

  Net& GetNetwork() const {  return *Network;  }
  void SetNetwork(Net& n)  {  Network = &n;  }
};
//---------------------------------------------------------------------------
...
public:
  TBasicBond(Net* P)
    : TSObject<Net>(P), FA(0), FB(0)
  {
    TSObject<Net>::Type = sotBBond;
  }
  virtual ~TBasicBond() {}
  bool IsValid() const { return FA != 0 && FB != 0; }
...
public:
  ...
  inline void NullNode(size_t i) { Nodes[i] = 0; }
  ...
  inline void NullBond(size_t i) { Bonds[i] = 0; }
