...
public:
  ...
  /* if do_throw is true throws an exception if size is larger than size_t
  (so that ne etc will fail) otherwise will return max size_t. Works ONLY for
  unsigned types!
  */
  template <typename IT>
  static IT CheckSize(uint64_t sz, bool do_throw = true) {
  ...
  /* if do_throw is true throws an exception if size is larger than size_t
  (so that ne etc will fail) otherwise will return max size_t
  */
...
protected:
  inline IInputStream& ValidateRead(uint64_t amount) {
    if ((GetPosition() + amount) <= GetSize()) {
      return *this;
    }
    TExceptionBase::ThrowFunctionFailed(__POlxSourceInfo, "invalid stream position");
