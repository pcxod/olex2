...
void LibFile::Which(TStrObjList& Cmds, const TParamList& Options, TMacroData& E)
{
  int mask = -1;
  ...
  olxstr rv = TEFile::Which(Cmds[0],
    Options.GetBoolOption("current_dir", false, true),
    Options.GetBoolOption("base_dir", false, true),
    mask);
...
TLibrary* LibFile::ExportLibrary(const olxstr& name) {
  ...
  lib->Register(
    new TStaticMacro(&LibFile::Which, "Which",
      "current_dir-checks current dir [true]&;"
      "base_dir-checks base dir [true]&;"
      "type-checks for file/dir [any]&;"
      ,
      fpAny^fpNone,
