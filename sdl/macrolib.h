...
public:
  ...
  bool ListCommands(const ArgsClass& args, CmdClass& cmds,
    TStrList& onAbort, TStrList& onListen,
    ...
    TStrList &argv)
  {
    ...
    argv.SetCount(Args.Count());
    for( size_t i=0; i < Args.Count(); i++ )
      olex_processor.processFunction(argv[i] = Args.GetObject(i));
    for( size_t i=0; i < args.Count(); i++ )  {
