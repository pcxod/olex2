  ...
  const uint16_t sF)
{
  ...
  while ((de = readdir(d)) != 0) {
    ...
    if (sF != sefAll) {
      if (access != 0) {
        int faccess = 0;
        if ((the_stat.st_mode & S_IRUSR) != 0) {
          faccess |= S_IRUSR;
        }
        if ((the_stat.st_mode & S_IWUSR) != 0) {
          faccess |= S_IWUSR;
        }
        if ((the_stat.st_mode & S_IXUSR) != 0) {
          faccess |= S_IXUSR;
        }
        if ((faccess & access) == 0) {
          continue;
      ...
      if ((sF & sefDir) != 0) {
        ...
        else if ((sF & sefFile) == 0) {
          continue;
        }
      }
      else if ((sF & sefFile) == 0 || S_ISDIR(the_stat.st_mode)) {
        continue;
      }
    }
...
TEFile* TEFile::TmpFile(const olxstr& templ) {
  if (templ.IsEmpty()) {
#ifdef __WIN32__
    olx_array_ptr<olxch> TmpFN(MAX_PATH);
