...
namespace exparse {
  ...
  class IEvaluableHolder : public IOlxObject {
    IEvaluable *val;
  public:
    IEvaluableHolder()
      : val(0)
    ...
    template <typename T> val_wrapper<T, IEvaluable> cast() const {
      if (val == 0) {
        throw TFunctionFailedException(__OlxSourceInfo, "null");
      }
      return val->cast<T>();
    }
  ...
  /* helper function to tackle 4.1 gcc unable to resolve 'const T& v' with
   v being a reference...
  */
