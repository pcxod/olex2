...
#ifndef __olx_sdl_math_H
#define __olx_sdl_math_H
#include "../ematrix.h"
#include "../bapp.h"
BeginEsdlNamespace()

/* most of the procedures are inspired by ALGLIB and LAPACK, some things, the
Householder reflection generation in particular - inspired by smtxb code by
Luc Bourhis
*/
namespace math  {
  ...
  namespace alg {
    ...
    void print0(const T& a, size_t m, const olxstr& annotation = EmptyString()) {
      ...
      TBasicApp::NewLogEntry() << olxstr(line);
    }
    template <typename T>
  ...
  struct LQ {
    ...
    static void Decompose(MatT& m, VecT& taus) {
      ...
      for (size_t i = 0; i < min_d; i++) {
        ...
        if (i < m.ColCount() - 1) {
          alg::copy_1(tmp, mr);
          ref.ApplyFromRight(m, taus(i), tmp,
            i + 1, m.RowCount(), i, m.ColCount());
        }
      }
    }
    template <typename MatIT, typename MatOT, typename VecT>
    static void UnpackQ(const MatIT& m, const VecT& taus, size_t row_cnt,
      MatOT& q)
    {
  ...
  struct QR {
    ...
    template <typename MatIT, typename MatOT, typename VecT>
    static void Unpack(const MatIT& qr, const VecT& tau, size_t col_num,
      MatOT& q)
    {
      typedef typename MatOT::number_type FT;
  ...
  struct Bidiagonal {
    ...
    template <typename MatIT, typename MatOT, typename VecT>
    static void UnpackQ(const MatIT& qp, const VecT& qtau, size_t col_cnt,
      MatOT& q)
    {
      typedef typename MatOT::number_type FT;
    ...
    template <typename MatIT, typename MatOT, typename VecT>
    static void UnpackPT(const MatIT& qp, const VecT& ptau, size_t row_cnt,
      MatOT& pt)
    {
      typedef typename MatOT::number_type FT;
    ...
    template <typename MatIT, typename MatOT, typename VecT>
    static void MulByQ(const MatIT& qp, const VecT& tauq, MatOT& z,
      bool from_right, bool dot_trans)
    {
      ...
      if (qp.RowCount() >= qp.ColCount()) {
        ...
        for (size_t i = i1; i < i2; i += step_inc) {
          alg::copy_1(tmp, proxy::col(qp, i, i, qp.RowCount()));
          if (from_right) {
            ref.ApplyFromRight(z, tauq(i), tmp, 0, z.RowCount(), i,
              qp.RowCount());
          }
          else {
            ref.ApplyFromLeft(z, tauq(i), tmp, i, qp.RowCount(), 0,
              z.ColCount());
          }
        }
      }
        ...
        for (size_t i = i1; i < i2; i += step_inc) {
          alg::copy_1(tmp, proxy::col(qp, i, i + 1, qp.RowCount()));
          if (from_right) {
            ref.ApplyFromRight(z, tauq(i), tmp, 0, z.RowCount(), i + 1,
              qp.RowCount());
          }
          else {
            ref.ApplyFromLeft(z, tauq(i), tmp, i + 1, qp.RowCount(), 0,
              z.ColCount());
          }
        }
      }
    ...
    template <typename MatIT, typename MatOT, typename VecT>
    static void MulByP(const MatIT& qp, const VecT& tauq, MatOT& z,
      bool from_right, bool dot_trans)
    {
      typedef typename MatOT::number_type FT;
      const size_t mx = olx_max(
      ...
      if (qp.RowCount() >= qp.ColCount()) {
        ...
        while (i1 != i2) {
          ...
          if (from_right) {
            ref.ApplyFromRight(z, tauq(i1), tmp, 0, z.RowCount(), i1 + 1,
              qp.ColCount());
          }
          else {
            ref.ApplyFromLeft(z, tauq(i1), tmp, i1 + 1, qp.ColCount(), 0,
              z.ColCount());
          }
          i1 += step_inc;
        }
        ...
        while (i1 != i2) {
          ...
          if (from_right) {
            ref.ApplyFromRight(z, tauq(i1), tmp, 0, z.RowCount(), i1,
              qp.ColCount());
          }
          else {
            ref.ApplyFromLeft(z, tauq(i1), tmp, i1, qp.ColCount(), 0,
              z.ColCount());
          }
          i1 += step_inc;
        }
    ...
    struct SVD {
      template <typename DiagT1, typename DiagT2, typename UT,
        typename CT, typename VtT>
        ...
        if (vt.ColCount() > 0) {
          Rotation<FT>::ApplyToRows(vt, i - 1, i, csr, snr);
        }
        if (u.RowCount() > 0) {
          Rotation<FT>::ApplyToCols(u, i - 1, i, csl, snl);
        }
        if (c.RowCount() > 0) {
          Rotation<FT>::ApplyToRows(c, i - 1, i, csl, snl);
        }
      }
      static void do2x2(FT f, FT g, FT h, FT& ssmin, FT& ssmax) {
        ...
        if (min_fh == 0) {
          ...
          ssmax = max_fh == 0 ? abs_g
            : mg * sqrt(1 + olx_sqr(olx_min(min_fh, abs_g) / mg));
        }
        else {
          if (abs_g < max_fh) {
          ...
          UT& u, CT& c, VtT& vt)
      {
        ...
        if (!upper) {
          for (size_t i = 0; i < d.Count() - 1; i++) {
            ...
            d(i + 1) *= cs;
            if (u.RowCount() > 0) {
              rot.ApplyToCols(u, i, i + 1, cs, sn);
            }
            if (c.ColCount() > 0) {
              rot.ApplyToRows(c, i, i + 1, cs, sn);
            }
          }
        }
        ...
        while (m != 0) {
          ...
          if ((flag == 1 && (dl_a < dm_a / 1000)) ||
            (flag == 2 && (dm_a < dl_a / 1000)))
          {
            change_dir = true;
          }
          ...
          if (flag == 1) {
            if (olx_abs(e(m - 1)) <= olx_abs(tol*d(m)) ||
              (tol < 0 && olx_abs(e(m - 1)) < thresh))
            {
              e(m - 1) = 0;
              continue;
            }
            ...
            if (olx_abs(e(ll)) <= olx_abs(tol*d(ll)) ||
              (tol < 0 && olx_abs(e(ll)) < thresh))
            {
              e(ll) = 0;
              continue;
            }
          ...
          if (shift == 0) {  // 0 shift - simplified QR iterations
            if (flag == 1) {  // chase bulge down
              ...
              for (size_t i = ll; i < m; i++) {
                ...
                if (u.RowCount() > 0) {
                  rot.ApplyToCols(u, i, i + 1, oldcs, oldsn);
                }
                if (c.ColCount() > 0) {
                  rot.ApplyToRows(c, i, i + 1, oldcs, oldsn);
                }
                d(i) = tmp;
              }
              const FT h = d(m)*cs;
              ...
              for (size_t i = m; i > ll; i--) {
                ...
                if (u.RowCount() > 0) {
                  rot.ApplyToCols(u, i - 1, i, cs, -sn);
                }
                if (c.ColCount() > 0) {
                  rot.ApplyToRows(c, i - 1, i, cs, -sn);
                }
              }
              const FT h = d(ll)*cs;
              d(ll) = h * oldcs;
            ...
            if (flag == 1) {  // chanse bulge down
              ...
              for (size_t i = ll; i < m; i++) {
                ...
                if (u.RowCount() > 0) {
                  rot.ApplyToCols(u, i, i + 1, cosl, sinl);
                }
                if (c.ColCount() > 0) {
                  rot.ApplyToRows(c, i, i + 1, cosl, sinl);
                }
              }
              ...
              for (size_t i = m; i > ll; i--) {
                ...
                if (u.RowCount() > 0) {
                  rot.ApplyToCols(u, i - 1, i, cosr, -sinr);
                }
                if (c.ColCount() > 0) {
                  rot.ApplyToRows(c, i - 1, i, cosr, -sinr);
                }
              }
    ...
    */
    ...
    bool Decompose(MatT& m, int u_flag = 2, int vt_flag = 2, bool extra_mem = false) {
      ...
      if (m.RowCount() > 1.6*m.ColCount()) {
        ...
        u.Transpose();
        bool res = Bidiagonal::SVD<FT>::Decompose(w, e, upper, false, zm, u, vt);
        u.Transpose();
        return res;
      }
      else {
        ...
        if (!extra_mem)
          ...
          bool res = Bidiagonal::SVD<FT>::Decompose(w, e, upper, false, zm, tmp, vt);
          for (size_t i = 0; i < min_d; i++) {
            for (size_t j = 0; j < m.RowCount(); j++) {
              u(j, i) = tmp(i, j);
            }
