...
void SHA2<T>::digest64(const uint32_t* msg)  {
  ...
  for( size_t i=16; i < 64; i++ )  {
    const uint32_t s0 = HashingUtils::hs_rotr(bf[i-15],7)^HashingUtils::hs_rotr(bf[i-15],18)^(bf[i-15] >> 3);
    const uint32_t s1 = HashingUtils::hs_rotr(bf[i-2],17)^HashingUtils::hs_rotr(bf[i-2],19)^(bf[i-2] >> 10);
    bf[i] = bf[i-16] + s0 + bf[i-7] + s1;
  }
  for( size_t i=0; i < 64; i++ )  {
    const uint32_t s0 = HashingUtils::hs_rotr(a,2)^HashingUtils::hs_rotr(a,13)^HashingUtils::hs_rotr(a,22);
    const uint32_t maj = (a&b) ^ (a&c) ^ (b&c);
    const uint32_t t2 = s0 + maj;
    const uint32_t s1 = HashingUtils::hs_rotr(e,6)^HashingUtils::hs_rotr(e,11)^HashingUtils::hs_rotr(e,25);
