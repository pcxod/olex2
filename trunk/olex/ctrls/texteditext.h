...
namespace ctrl_ext  {
  ...
  public:
    TTextEdit(wxWindow *Parent, int style=0) :
      ...
      OnLeave(Actions.NewQueue(evt_on_mouse_leave_id)),
      OnEnter(Actions.NewQueue(evt_on_mouse_enter_id)),
      OnReturn(Actions.NewQueue(evt_on_return_id)),
      OnClick(Actions.NewQueue(evt_on_click_id)),
      OnChar(Actions.NewQueue(evt_on_char_id)),
      OnKeyDown(Actions.NewQueue(evt_on_key_down_id)),
      Data(EmptyString),
