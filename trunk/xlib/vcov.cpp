...
void VcoVMatrix::ReadShelxMat(const olxstr& fileName)  {
  Clear();
  TCStrList sl, toks;
  ...
  for( int i=1; i < cnt; i++ )  { // skipp OSF
    ...
    if( toks[4].CharAt(0) == 'x' )  {
      ...
      Index.AddNew( toks[5], vcoviX );
      indexes.Add(i);
    }
    else if( toks[4].CharAt(0) == 'y' )  {
      diag.Add(toks[2].ToDouble());
      Index.AddNew( toks[5], vcoviY );
      indexes.Add(i);
    }
    else if( toks[4].CharAt(0) == 'z' )  {
      diag.Add(toks[2].ToDouble());
      Index.AddNew( toks[5], vcoviZ );
      indexes.Add(i);
    }
    else if( toks[4] == sof )  {
      diag.Add(toks[2].ToDouble());
      Index.AddNew( toks[5], vcoviO );
      indexes.Add(i);
    }
  }
  ...
  for( int i=0; i < indexes.Count(); i++ )  {
    for( int j=i; j < indexes.Count(); j++ )  {
      if( i == j )  
        ...
        data[j][i] = all_vcov[ indexes[j]*cnt/2 + indexes[i] ];  
    }
  }
}
double VcoVMatrix::Find(const olxstr& atom, const short va, const short vb) const {
  for( int i=0; i < Index.Count(); i++ )  {
    if( Index[i].GetA() == atom )  {
      int i1 = -1, i2 = -1;
      for( int j=i; Index[j].GetA() == atom; j++ )  {
