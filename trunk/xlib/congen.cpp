  ...
  switch( Group )  {
    case fgCH3:
      if( envi.Count() == 1 )  {
        ...
        if( NEnvi.Count() >= 2 )  {
          RotVec = NEnvi.GetBase().crd();
          RotVec -= envi.GetBase().crd();
          RotVec.Normalise();
    ...
    case fgOH2:
      if( envi.IsEmpty() && pivoting != NULL)  {
        if( pivoting->Count() == 2 )  {
          ...
          Vec1 -= pivoting->GetBase().crd();
          Vec1.Normalise();
          Vec1 *= 0.85;
          crds.AddNew( pivoting->GetBase().crd() );
          crds[0] += Vec1;
