...
void TNetwork::CreateBondsAndFragments(TSAtomPList& Atoms, TNetPList& Frags, TSBondPList& bond_sink)  {
    ...
    for( size_t j=0; j < ci.BondsToRemove.Count(); j++ )  {
      if( ci.BondsToRemove[j].matr == NULL )  {
        ...
        for( size_t k=0; k < sa->NodeCount(); k++ )  {
          if( sa->Node(k).CAtom() == ci.BondsToRemove[j].to )  {
            bool remove = false;
            for( size_t l=0; l < sa->Node(k).MatrixCount(); l++ )  {
    ...
    for( size_t j=0; j < ci.BondsToCreate.Count(); j++ )  {
      if( ci.BondsToCreate[j].matr == NULL )  {
        ...
        for( size_t k=0; k < ac; k++ )  {
          if( Atoms[k]->CAtom() == ci.BondsToCreate[j].to )  {
            bool add = false;
            for( size_t l=0; l < Atoms[k]->MatrixCount(); l++ )  {
  ...
  for( size_t i=0; i < ac; i++ )  {
    ...
    if( A1->NodeCount() > (size_t)olx_abs(ci.maxBonds) )  {
      if( ci.maxBonds < 0 )
...
//      return -1;
  ...
  void HValidator( TEGraphNode<size_t,TSAtom*>& n1, TEGraphNode<size_t,TSAtom*>& n2)  {
    ...
    TSizeList new_pos(hpos.Count()), Used;
    for( size_t i=0; i < hpos.Count(); i++ )  {
      ...
      for( size_t j=0; j < hpos.Count(); j++ )  {
        if( Used.IndexOf(j) != InvalidIndex )  continue;
...
bool TNetwork_TryRing( TSAtom* sa, TSAtomPList& ring, const TPtrList<TBasicAtomInfo>& ringContent, int level=1 )  {
  ...
  for( size_t i=0; i < sa->NodeCount(); i++ )  {
    ...
    if( level >= (int)ringContent.Count() && a.GetTag() == 1 )
      return true;
    if( a.GetTag() != 0 && a.GetTag() < level )  continue;
    if( level < (int)ringContent.Count() && TNetwork_TryRing(&a, ring, ringContent, level+1) ) {
      ring.Add( &a );
      return true;
    }
  }
...
bool TNetwork_TryRing( TSAtom& sa, size_t node, TSAtomPList& ring, const TPtrList<TBasicAtomInfo>& ringContent )  {
  sa.SetTag(1);
...
int TNetwork_SortRingAtoms( const TSAtom* a, const TSAtom* b )  {
  return olx_cmp_size_t(a->GetTag(), b->GetTag());
}
