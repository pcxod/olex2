...
void TMainForm::macPictPR(TStrObjList &Cmds, const TParamList &Options, TMacroError &Error)  {
  ...
  TCStrList(FXApp->ToPov()).SaveToFile(file_name);
}
//..............................................................................
...
void TMainForm::macMpln(TStrObjList &Cmds, const TParamList &Options, TMacroError &Error)  {
  ...
  if( plane != NULL )  {
    ...
    const smatd im = Atoms[0]->GetMatrix(0).Inverse();
    double rmsd = 0;
    ...
    TBasicApp::NewLogEntry() << tab.CreateTXTList(olxstr("Atom-to-plane distances for ") << planeName,
      true, false, " | ");
    TBasicApp::NewLogEntry() << "Plane normal: " << olxstr::FormatFloat(3, plane->GetNormal()[0])
      << ' ' << olxstr::FormatFloat(3, plane->GetNormal()[1]) << ' '
      << olxstr::FormatFloat(3, plane->GetNormal()[2]);
    if( weightExtent != 0 )  {
      TBasicApp::NewLogEntry() << "Weighted RMSD/A: " << olxstr::FormatFloat(3, plane->GetWeightedRMSD());
      TBasicApp::NewLogEntry() << "RMSD/A: " << olxstr::FormatFloat(3, plane->CalcRMSD());
    }
    else  {
      TBasicApp::NewLogEntry() << "RMSD/A: " << olxstr::FormatFloat(3, plane->GetWeightedRMSD());
    }
  }
  else if( !orientOnly )
    TBasicApp::NewLogEntry() << "The plane was not created because it is either not unique or valid";
}
//..............................................................................
...
void TMainForm::macLabel(TStrObjList &Cmds, const TParamList &Options, TMacroError &E)  {
  ...
  for( size_t i=0; i < atoms.Count(); i++ )  {
    ...
    if( lt != 0 && atoms[i]->GetLabel().Length() > atoms[i]->GetType().symbol.Length() )  {
      olxstr bcc = atoms[i]->GetLabel().SubStringFrom(atoms[i]->GetType().symbol.Length());
...
void TMainForm::macAfix(TStrObjList &Cmds, const TParamList &Options, TMacroError &E)  {
  ...
  if( TAfixGroup::IsFittedRing(afix) )  {  // special case
    ...
    if( Atoms.IsEmpty() )
      ...
      FXApp->AutoAfixRings(afix, Atoms[0], Options.Contains('n'));
    else  {
...
void TMainForm::macDfix(TStrObjList &Cmds, const TParamList &Options, TMacroError &E)  {
  ...
  if( Atoms.Count() == 1 )  {  // special case
    ...
    for( size_t i=0; i < XA->NodeCount(); i++ )  {
      ...
      dfix->AddAtomPair(XA->CAtom(), &XA->GetMatrix(0), SA->CAtom(), &SA->GetMatrix(0));
    }
  }
...
void TMainForm::macTria(TStrObjList &Cmds, const TParamList &Options, TMacroError &E)  {
  ...
  if( xatoms.IsEmpty() )  {
    ...
    if( sel.Count() > 1 ) {
      for( size_t i=0; i < sel.Count(); i += 2 )  {
        ...
        const TXBond& xba = (TXBond&)sel[i];
        const TXBond& xbb = (TXBond&)sel[i+1];
        TXAtom* xa = (&xba.A() == &xbb.A() || &xba.B() == &xbb.A() ) ? &xbb.A() : 
                    ((&xba.A() == &xbb.B() || &xba.B() == &xbb.B()) ? &xbb.B() : 
                      NULL );
        if( xa == NULL )  {
          E.ProcessingError(__OlxSrcInfo, "some bonds do not share atom");
          return;
        }
        xatoms.Add(xba.Another(*xa));
        xatoms.Add(xa);
        xatoms.Add(xbb.Another(*xa));
      }
    }
    else
      E.ProcessingError(__OlxSrcInfo, "no atoms or bonds provided");
  ...
  for( size_t i=0; i < xatoms.Count(); i+=3 )  {
    if( dfixLenA == 0 )  {
      dfixLenA = FXApp->XFile().GetRM().FindRestrainedDistance(
        xatoms[i]->CAtom(), xatoms[i+1]->CAtom());
      dfixLenB = FXApp->XFile().GetRM().FindRestrainedDistance(
        xatoms[i+1]->CAtom(), xatoms[i+2]->CAtom());
      if( dfixLenA == -1 || dfixLenB == -1 )  {
        TBasicApp::NewLogEntry(logError) << "Tria: please fix or provided distance(s)";
        dfixLenA = dfixLenB = 0;
        continue;
      }
    }
...
void TMainForm::macCalcVoid(TStrObjList &Cmds, const TParamList &Options, TMacroError &E)  {
  ...
  //
  if( catoms.IsEmpty() )
    TBasicApp::NewLogEntry() << "Calculating for all atoms of the asymmetric unit";
  else  {
  ...
  //TBasicApp::NewLogEntry() << "Voids volume (A^3) " << olxstr::FormatFloat(3, (mapVol-_pc)*vol/mapVol);
  //TBasicApp::NewLogEntry() << "Max level reached ") << MaxLevel << '\n');
...
void TMainForm::macMatch(TStrObjList &Cmds, const TParamList &Options, TMacroError &E)  {
  ...
  TXAtomPList atoms = FindXAtoms(Cmds, false, true);
  if( !atoms.IsEmpty() )  {
    if( atoms.Count() == 2 && (&atoms[0]->GetNetwork() != &atoms[1]->GetNetwork()) )  {
      ...
      if( match )  {
        ...
        if( subgraph )  {
          sk.Add( res[0].GetB() );
          res.Clear();
          while( atoms[0]->GetNetwork().IsSubgraphOf( atoms[1]->GetNetwork(), res, sk ) )  {
            sk.Add( res[0].GetB() );
            tmp = '=';
            for( size_t i=0; i < res.Count(); i++ )  {
              tmp << '{' << atoms[0]->GetNetwork().Node(res[i].GetA()).GetLabel() <<
...
void TMainForm::macIT(TStrObjList &Cmds, const TParamList &Options, TMacroError &Error)  {
  ...
  TBasicApp::NewLogEntry() << "Inertion tensor:";
  for( size_t i=0; i < 3; i++ )
    TBasicApp::NewLogEntry() << I[i].ToString();
  mat3d::EigenValues(I, Iv);
  TBasicApp::NewLogEntry() << "Ixx =  " << olxstr::FormatFloat(3, I[0][0]) <<
                              "  Iyy = "  << olxstr::FormatFloat(3, I[1][1]) <<
                              "  Izz = "  << olxstr::FormatFloat(3, I[2][2]);
  TBasicApp::NewLogEntry() << "Eigen vectors:";
  for( size_t i=0; i < 3; i++ )
    TBasicApp::NewLogEntry() << Iv[i].ToString();
  ...
  if( sel.Count() != 0 )  {
    if( sel.Count() == 1 )  {
      if( EsdlInstanceOf(sel[0], TXAtom) )  {
        ...
        TBasicApp::NewLogEntry() << (values.Add(xb.A().GetLabel()) << " to " <<
          xb.B().GetLabel() << " distance: " <<
          vcovc.CalcDistance(xb.A(), xb.B()).ToString() << " A");
      }
    }
    else if( sel.Count() == 2 )  {
      if( EsdlInstanceOf(sel[0], TXAtom) && EsdlInstanceOf(sel[1], TXAtom) )  {
        TBasicApp::NewLogEntry() << (values.Add(((TXAtom&)sel[0]).GetLabel()) << " to " <<
        ...
        TBasicApp::NewLogEntry() << (values.Add(b1.A().GetLabel()) << '-' << b1.B().GetLabel() << " to " <<
        ...
        TBasicApp::NewLogEntry() << (values.Add(sa.GetLabel()) << " to plane " << pld << "distance: " <<
          vcovc.CalcP2ADistance(atoms, sa).ToString() << " A");
        TBasicApp::NewLogEntry() << (values.Add(sa.GetLabel()) << " to plane " << pld << "centroid distance: " <<
        ...
        TEValue<double> v(vcovc.CalcP2VAngle(atoms, sb.A(), sb.B())),
          v1(180-v.GetV(), v.GetE());
        TBasicApp::NewLogEntry() << (values.Add(sb.A().GetLabel()) << '-' << sb.B().GetLabel() << " to plane "
        ...
        TBasicApp::NewLogEntry() << (values.Add("Plane ") << pld1 << "to plane angle: " <<
          angle.ToString());
        TBasicApp::NewLogEntry() << (values.Add("Plane centroid to plane centroid distance: ") <<
          vcovc.CalcPC2PCDistance(p1, p2).ToString() << " A");
        TBasicApp::NewLogEntry() << (values.Add("Plane [") << pld1 << "] to plane centroid distance: " <<
          vcovc.CalcP2PCDistance(p1, p2).ToString() << " A");
        TBasicApp::NewLogEntry() << (values.Add("Plane [") << pld1 << "] to plane shift: " <<
          vcovc.CalcP2PShiftDistance(p1, p2).ToString() << " A");
        if( olx_abs(angle.GetV()) > 1e-6 )  {
          TBasicApp::NewLogEntry() << (values.Add("Plane [") << pld2 << "] to plane centroid distance: " <<
            vcovc.CalcP2PCDistance(p2, p1).ToString() << " A");
          TBasicApp::NewLogEntry() << (values.Add("Plane [") << pld2 << "] to plane shift distance: " <<
      ...
      if( EsdlInstanceOf(sel[0], TXAtom) && EsdlInstanceOf(sel[1], TXAtom) && EsdlInstanceOf(sel[2], TXAtom) )  {
        ...
        TBasicApp::NewLogEntry() << (values.Add(a1.GetLabel()) << '-' << a2.GetLabel() << '-' << a3.GetLabel()
        ...
        TBasicApp::NewLogEntry() << (values.Add(a1->GetLabel()) << '-' << a2->GetLabel() << " to plane " << pld <<
      ...
      if( face_cnt == 8 )  {
        TBasicApp::NewLogEntry() << "Combined distortion (best plane): " <<
