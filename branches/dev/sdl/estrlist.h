...
public:
  ...
  void QSort(bool ci)  {
    if( ci )  TPtrList<T>::QuickSorter.Sort<TStringWrapperComparator<T,true> >(Strings);
    else      TPtrList<T>::QuickSorter.Sort<TStringWrapperComparator<T,false> >(Strings);
  }
  ...
  void Hyphenate(const SC& String, size_t Width, bool Force=true)  {
    if( Width == 0 )
