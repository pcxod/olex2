...
protected:
  ...
  static void peak_search(const array_3d<MapT> &data,
    MapT pos_level, const array_3d<bool>& mask, TArrayList<peak>& maxima)
  {
    TStack<vec3i> stack;
    vec3s dim = data.dim();
...
public:
  ...
  template <typename MapT> static void Integrate(
    const array_3d<MapT> &map,
    const array_3d<bool> &Mask,
    MapT pos_level, TArrayList<MapUtil::peak>& Peaks)
  {
    const vec3i map_dim = map.dim();
    const vec3i dim_e = Mask.dim();
    const MapT neg_level = -pos_level;
    ...
    peak_search<MapT>(map, pos_level, Mask, Peaks);
  }
  // a simple map integration, considering the peaks and holes as spheres
  template <typename MapT> static void Integrate(
    const array_3d<MapT> &map, MapT pos_level,
    TArrayList<MapUtil::peak>& Peaks)
  {
    array_3d<bool> Mask(map.width, map.height, map.depth);
    Integrate(map, Mask, pos_level, Peaks);
  }
  // a simple map integration, considering the peaks and holes as spheres
  template <typename MapT> static double IntegrateMask(const array_3d<MapT> &map,
  ...
  */
  template <typename map_type> static vec3i AnalyseChannels1(
    array_3d<map_type> &map,
    map_type max_level)
  {
    array_3d<map_type> map_copy = map;
    vec3i dim_ind(0, 1, 2);
    vec3s dim = map.dim();
    vec3i res;
  ...
  */
  ...
  static ConstTypeList<vec3d> FindLevelCenters(array_3d<map_type> &map,
    map_type level, map_type safe_value)
  {
    const vec3i dim(map.dim());
    vec3d_list centers;
...
public:
  ...
  template <typename mapT, int type> struct MapGetter {
    ...
    MapGetter(const array_3d<mapT> &_src) : src(_src), dim(_src.dim())
    {}
    mapT Get(const TVector3<mapT>& fractional_crd) const {
      const TVector3<mapT> p(fractional_crd[0] * dim[0],
        fractional_crd[1] * dim[1], fractional_crd[2] * dim[2]);
        ...
        for (int dx = 0; dx <= 1; dx++) {
          ...
          for (int dy = 0; dy <= 1; dy++) {
            ...
            for (int dz = 0; dz <= 1; dz++) {
              ...
              const vec3i i = NormaliseIndex(
                vec3d(fp[0] + dx, fp[1] + dy, fp[2] + dz));
      ...
      if (type == 2) {  // cubic interpolation
        ...
        const mapT vx[4] = { -_pc / 6 + _ps / 2 - _p / 3, (_pc - _p) / 2 - _ps + 1,
          (-_pc + _ps) / 2 + _p, (_pc - _p) / 6 };
        const mapT vy[4] = { -_qc / 6 + _qs / 2 - _q / 3, (_qc - _q) / 2 - _qs + 1,
          (-_qc + _qs) / 2 + _q, (_qc - _q) / 6 };
        const mapT vz[4] = { -_rc / 6 + _rs / 2 - _r / 3, (_rc - _r) / 2 - _rs + 1,
          (-_rc + _rs) / 2 + _r, (_rc - _r) / 6 };
  ...
  cell
  */
  template <class _MapGetter, typename dest_t> static array_3d<dest_t> &
