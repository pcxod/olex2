...
//    TBasicApp::GetLog() << "Please press Ctrl+C to terminate a process...\n";
  ...
  void macInfo(TStrObjList &Cmds, const TParamList &Options, TMacroError &Error)  {
    ...
    for(int i = 0; i < atoms.Count(); i++ )  {
      ...
      Table[i][2] = olxstr::FormatFloat(3, atoms[i]->CCenter()[0].GetV());
      Table[i][3] = olxstr::FormatFloat(3, atoms[i]->CCenter()[1].GetV());
      Table[i][4] = olxstr::FormatFloat(3, atoms[i]->CCenter()[2].GetV());
