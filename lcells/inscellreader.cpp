...
bool _fastcall TInsCellReader::LoadFromInsFile(const olxstr& FN)  {
  ...
  for( int i=0; i < S.Count(); i++ )  {
    ...
    if( toks.Count() == 0 )
      continue;
    if( toks.String(0) == "cell" && toks.Count() == 8 )  {
      Fa = toks[2].ToDouble();
      Fb = toks[3].ToDouble();
      Fc = toks[4].ToDouble();
      Faa = toks[5].ToDouble();
...
bool _fastcall TInsCellReader::LoadFromCifFile(const olxstr& FN)  {
  ...
  for( int i=0; i < S.Count(); i++ )  {
    if( S.String(i).IsEmpty() || S.String(i)[0] != '_' )  continue;
    toks.Clear();
    toks.Strtok(S.String(i).LowerCase(), ' ');
    if( toks.Count() < 2 )  continue;

    if( toks.String(0) == "_cell_length_a" )
      Fa = toks[1].ToDouble();
    ...
    if( toks[0] == "_symmetry_space_group_name_h-m" )  {
      for( int j=0; j < toks.String(1).Length(); j++ )  {
