...
protected:
  struct CellEsd {
    ...
    template <class Evaluator> struct Functor {
      ...
      Functor(CellEsd& _base, const Evaluator& e)
        : base(_base), eval(e)
      {}
      double calc() const {
...
public:
  ...
  template <class pT, class cT> struct Centroid2CentriodShiftDistance {
    const pT& p;
    const cT& c;
    Centroid2CentriodShiftDistance(const pT& _p, const cT& _c)
      : p(_p), c(_c)
    {}
    double calc() const {
  ...
  template <class VT, class WT> struct CentroidEvaluator {
    const VT& values;
    const WT& weights;
    CentroidEvaluator(const VT& _v, const WT& _w)
      : values(_v), weights(_w)
    {}
  ...
  struct VectorProxy {
    vec3d &a, &b;
    VectorProxy(vec3d& _a, vec3d& _b)
      : a(_a), b(_b)
    {}
    vec3d evaluate() const { return b - a; }
  };
  /* Calculates the right-handed normal */
  template <class VT, class WT> struct PlaneNormalEvaluator {
    VT values;
    WT weights;
    bool inverse;
    PlaneNormalEvaluator(const VT& _v, const WT& _w, const vec3d &n)
    ...
    vec3d evaluate() const {
      PlaneInfo pi = CalcPlane(values, weights);
      return inverse ? -pi.normal : pi.normal;
    }
  };
  ...
  template <class PT> struct TwistAngle {
    PT a, b;
    TwistAngle(const PT &a_, const PT &b_)
      : a(a_), b(b_)
    {}
    double calc() const {
  ...
  template <class PT> struct FoldAngle {
    PT a, b;
    FoldAngle(const PT &a_, const PT &b_)
      : a(a_), b(b_)
    {}
    double calc() const {
      const PlaneInfo ai = a.evaluate(), bi = b.evaluate();
  ...
  template <class aT, class bT, class cT> struct Angle3 {
    ...
    bT b;
    cT c;
    Angle3(const aT& _a, const bT& _b, const cT& _c)
      : a(_a), b(_b), c(_c)
    {}
    double calc() const {
  ...
  template <class aT, class bT> struct Angle2 {
    aT a;
    bT b;
    Angle2(const aT& _a, const bT& _b)
      : a(_a), b(_b)
    {}
    double calc() const {
  ...
  template <class aT, class bT> struct Distance {
    aT a;
    bT b;
    Distance(const aT& _a, const bT& _b)
      : a(_a), b(_b)
    {}
...
public:
  ...
  // returns rmsd for the best plane
